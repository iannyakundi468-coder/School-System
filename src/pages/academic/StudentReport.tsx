                    ...
                    <div className="flex flex-col md:flex-row gap-6 items-center md:items-start text-center md:text-left">
                        ...
                        <div className="flex-1">
                            ...
                            <div className="flex flex-wrap justify-center md:justify-start gap-4 text-sm text-gray-400">
                                <span className="flex items-center gap-1"><BookOpen className="w-4 h-4 text-indigo-400" /> {student.id}</span>
                                <span className="flex items-center gap-1"><Users className="w-4 h-4 text-sky-400" /> {student.class}</span>
                            </div>
                        </div>
                        <div className="flex gap-8 text-center bg-slate-800/50 p-4 rounded-xl border border-slate-700">
                            <div>
                                ...
                                <p className="text-4xl font-bold text-indigo-400">{student.overallGrade}</p>
                            </div>
                            <div className="w-px bg-slate-700"></div>
                            <div>
                                <p className="text-gray-400 text-xs uppercase tracking-wider mb-1">Attendance</p>
                                <p className="text-4xl font-bold text-sky-400">{student.attendance}%</p>
                            </div>
                        </div>
                    </div>
                </Card>
                ...
                <div className="grid md:grid-cols-3 gap-6">
                    ...
                    <Card className="md:col-span-2">
                        <h3 className="text-xl font-bold mb-6 flex items-center gap-2">
                            <TrendingUp className="w-5 h-5 text-indigo-400" /> Subject Performance
                        </h3>
                            ...
                            {subjects.map((subject) => (
                                <div key={subject.name} className="group">
                                    <div className="flex justify-between items-end mb-2">
                                        <div>
                                            <h4 className="font-bold text-white group-hover:text-indigo-400 transition-colors">{subject.name}</h4>
                                            <p className="text-xs text-gray-500">{subject.teacher}</p>
                                        </div>
                                        <div className="text-right">
                                            <span className="text-xl font-bold text-white">{subject.score}%</span>
                                            <span className={`ml-2 text-xs px-2 py-0.5 rounded ${subject.grade === 'A' || subject.grade === 'A-' ? 'bg-indigo-500/20 text-indigo-400' :
                                                subject.grade.startsWith('B') ? 'bg-sky-500/20 text-sky-400' :
                    ...
                    <div className="space-y-6">
                        <Card>
                            <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
                                <Clock className="w-5 h-5 text-sky-400" /> Attendance Log
                            </h3>
                            <div className="flex items-center justify-center py-6">
                                <div className="relative w-40 h-40">
                                    ...
                                    <div className="w-full h-full rounded-full border-8 border-slate-700 flex items-center justify-center relative">
                                        <div className="absolute inset-0 rounded-full border-8 border-sky-500 border-t-transparent border-l-transparent rotate-45 transform"></div>
                            ...
                            <div className="grid grid-cols-2 gap-4 text-center text-sm">
                                <div className="bg-slate-800/50 p-2 rounded-lg">
                                    <span className="block text-indigo-400 font-bold">58</span>
                            ...
                            <div className="space-y-4">
                                {comments.map((item, idx) => (
                                    <div key={idx} className="bg-slate-800/30 p-3 rounded-lg border border-slate-700/50">
                                        <p className="text-xs text-indigo-400 font-bold mb-1 uppercase">{item.teacher}</p>
